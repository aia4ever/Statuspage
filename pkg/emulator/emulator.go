...
const accidentListFilename = "accidents.data"
...
var AccidentCollection []AccidentItem
...
type AccidentItem struct {
	Topic  string `json:"topic"`
	Status string `json:"status"`
}

const accidentStatusActive = "active"
const accidentStatusClosed = "closed"

var AccidentTopics = []string{
	"SMS delivery in EU",
...
func Shuffle() {
	...
	AccidentCollection = shuffleAccidentData()
...
func shuffleAccidentData() []AccidentItem {
	collection := make([]AccidentItem, 0)
	status := ""
	for _, topic := range AccidentTopics {
		if getRandomIntBetweenValues(0, 1) == 1 {
			status = accidentStatusActive
		} else {
			status = accidentStatusClosed
		}

		collection = append(collection, AccidentItem{Topic: topic, Status: status})
	}
...
func handleAccident(w http.ResponseWriter, r *http.Request) {
	response(w, r, AccidentCollection)
}
